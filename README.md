...
| Tool Name | Description |
|-----------|-------------|
| `performance_<db_id>` | Analyze query performance and get optimization suggestions. Actions: `suggest_indexes` (alias-safe CREATE INDEX DDL from JOIN/WHERE/ORDER BY/GROUP BY columns vs live catalogs, composite-aware, PK-aware), `engine_slow_queries` (top statements from `pg_stat_statements` / MySQL digests), `list_sessions` (active sessions from pg_stat_activity / processlist), `lock_waits` (who blocks whom via pg_blocking_pids / sys.innodb_lock_waits), `long_transactions` with optional `min_age_secs` (idle-in-transaction sessions holding locks and blocking vacuum, oldest first), `replication_status` (attached replicas with replay position/lag via pg_stat_replication / SHOW REPLICA STATUS; empty = no replicas attached), `connection_saturation` (engine connections vs max_connections; ≥80% warning, ≥95% critical — "too many clients" incidents become visible before they fire), `timeout_guardrails` (engine-side runaway-query ceilings: statement_timeout, idle_in_transaction_session_timeout, max_execution_time; zeros flagged UNPROTECTED), `idle_sessions` (connections doing nothing while eating the client ceiling — pool leaks and forgotten shells, oldest first; complements list_sessions which shows only activity), `temp_spills` (sorts/temp tables spilling to disk — the undersized-work_mem signal; Postgres temp_files/bytes, MySQL on-disk ratio with verdict), `seq_scan_heavy` (per-table sequential-vs-index scan counters from pg_stat_user_tables / performance_schema — workload-based evidence of where missing indexes actually hurt), `stale_slots` (inactive replication slots retaining WAL — the silent disk-full incident; Postgres), `deadlock_counts` (cumulative engine deadlocks since stats reset — durable evidence a concurrency conflict is firing; victim queries live in engine logs), `wraparound_risk` (per-database transaction-ID age from datfrozenxid, worst first — Postgres stops accepting writes near 2B and this is the only warning; WARNING at ≥200M, CRITICAL at ≥500M), `charset_audit` (columns still on deprecated utf8mb3 with table/charset/collation and a conversion hint — MySQL; the emoji/index-length migration landmine), `list_extensions` (installed extensions with versions plus available-but-not-installed — why engine_slow_queries says unsupported becomes checkable; Postgres), `prepared_xacts` (in-doubt two-phase transactions with owner and age — a forgotten PREPARE TRANSACTION holds locks forever and pins vacuum, invisible to session listings; Postgres), `wal_archive` (archiver success/failure counters with the last failed segment — a failing archive_command accumulates WAL until the disk fills and silently breaks point-in-time recovery; Postgres), `autovacuum_disabled` (tables whose storage parameters turn autovacuum off — dead tuples and XID-wraparound age accumulate silently there; Postgres), `checkpoint_pressure` (timed vs requested checkpoint split — a high requested ratio means max_wal_size is forcing aggressive checkpoints and writes pay latency spikes; PG17+ and legacy views), `invalid_indexes` (planner-ignored indexes left by crashed CREATE INDEX CONCURRENTLY — no read benefit, full write cost, with REINDEX/DROP fix; Postgres), `role_connection_limits` (capped login roles at or near their rolconnlimit — a role at its limit rejects every new session while others log in fine; Postgres), `foreign_tables` (FDW servers and the local table names proxying to remote systems — a query against one silently crosses to another database; Postgres), `unlogged_tables` (WAL-skipping tables — truncated on crash recovery and never replicated to standbys, with SET LOGGED fix; Postgres), `myisam_tables` (tables still on the MyISAM engine — no transactions, table-level locks, corruption-prone on unclean shutdown, with ENGINE=InnoDB migration hint; MySQL/MariaDB), `unpopulated_matviews` (materialized views that error on every query because they were never populated, with REFRESH fix; Postgres), `binlog_growth` (binary-log total size vs retention — expire_logs_seconds=0 silently fills the disk, with the setting fix; MySQL/MariaDB), `auto_increment_headroom` (auto-increment counters at or near their type ceiling — inserts fail at 100%, with BIGINT migration hint; MySQL/MariaDB), `slow_log` (whether the engine would even record a slow query — disabled logging is blindness, names the exact SET fix; MySQL/MariaDB/Postgres), `password_auth` (login roles still carrying broken md5 hashes plus the server hashing default, with ALTER ROLE fix and privilege notes; Postgres), `table_cache` (MySQL table_open_cache saturation — full cache with high Opened_tables churn means every access pays a reopen; names the SET GLOBAL fix; MySQL/MariaDB), `durability` (whether committed transactions survive a crash — innodb_flush_log_at_trx_commit 0/2 can lose ~1s of commits; names the fix; MySQL/MariaDB), `strict_mode` (STRICT_TRANS_TABLES absent means overlong strings silently truncate and invalid dates coerce — corruption that surfaces weeks later; names the SET GLOBAL fix; MySQL/MariaDB), `aborted_connections` (failed handshakes → hosts get BLOCKED at max_connect_errors; unclean client exits → pool teardown issues; ratio-based; MySQL/MariaDB), `max_packet` (max_allowed_packet below 16MB means big blob/batch writes fail with "server has gone away" — names the SET GLOBAL + client DSN fix; MySQL/MariaDB), `wal_mode` (SQLite journal mode: rollback journal blocks readers during writes → SQLITE_BUSY under concurrent access; names the PRAGMA fix), `synchronous_commit` (off means acknowledged commits can be lost on crash — the PostgreSQL counterpart of the durability check; names the ALTER SYSTEM fix; PostgreSQL), `busy_timeout` (SQLite default 0ms means lock contention fails immediately with SQLITE_BUSY instead of waiting; names the PRAGMA fix; SQLite), `track_io_timing` (off hides I/O timings in EXPLAIN ANALYZE and pg_stat views — can't tell CPU-bound from disk-bound; names the ALTER SYSTEM fix; PostgreSQL), `wait_timeout` (two-sided: too high lets crashed clients' idle connections hold slots until 'too many connections', too low causes 'server has gone away' churn; MySQL/MariaDB), `buffer_pool` (compares innodb_buffer_pool_size against total data+index bytes so an undersized cache is visible before hit rates degrade; names the SET GLOBAL fix; MySQL/MariaDB), `fk_enforcement` (SQLite parses FOREIGN KEY constraints but doesn't enforce them unless PRAGMA foreign_keys=ON per connection — off means orphaned child rows are written silently; names the PRAGMA fix; SQLite), `crash_safety` (fsync=off loses acknowledged commits on power failure, full_page_writes=off invites torn-page corruption after a crash — benchmark settings that must never reach production; names the ALTER SYSTEM fix; PostgreSQL), `wal_level` (minimal disables streaming replication, point-in-time recovery, and logical decoding — a primary crash past archived WAL becomes unrecoverable; names the ALTER SYSTEM fix; PostgreSQL), `shared_buffers` (the default 128MB pool is famously undersized — compares the pool against pg_database_size so cold-read pressure is visible before hit ratios degrade; names the ALTER SYSTEM + restart fix; PostgreSQL), `open_files_limit` (a file-descriptor ceiling below ~2x table_open_cache silently caps the cache — evictions and "Too many open files" follow no matter how the cache is tuned; needs config + restart, not SET GLOBAL; MySQL/MariaDB), `binlog_format` (STATEMENT replication silently diverges on NOW()/UUID()/unordered LIMIT — MIXED still routes unsafe shapes; ROW is the only deterministic format; names the SET GLOBAL fix; MySQL/MariaDB), `doublewrite` (innodb_doublewrite=OFF — the benchmark tweak — leaves the tablespace open to torn-page corruption that only surfaces when a backup restore exercises it; names the config fix; MySQL/MariaDB), `flush_method` (innodb_flush_method=fsync double-buffers every write into buffer pool + OS page cache — wasted RAM and checkpoint stalls; O_DIRECT is the Linux fix; names the config + restart path; MySQL/MariaDB), `io_capacity` (innodb_io_capacity=200 is the single-spinning-disk default — lazy flushing until dirty pages pile up, then write stalls in bursts; names the live SET GLOBAL fix; MySQL/MariaDB), `wal_compression` (wal_compression=off floods WAL with raw full-page images every checkpoint — replica lag and ballooning archives; lz4/zstd are the cheap PG14+ fix, reload not restart; PostgreSQL), `redo_log` (undersized InnoDB redo log forces aggressive checkpointing and write amplification — the 48MB×2 legacy default still bites migrated servers; names the live SET GLOBAL innodb_redo_log_capacity fix; MySQL/MariaDB), `slot_wal_cap` (max_slot_wal_keep_size=-1 lets one forgotten replication slot retain WAL until pg_wal fills the disk — a cap converts that into bounded slot invalidation; PostgreSQL), `autovacuum_throttle` (the legacy spinning-disk cost budget — 2ms delay / limit 200 — lets busy write-heavy tables outpace vacuum, so bloat accumulates invisibly; names the ALTER SYSTEM fix; PostgreSQL), `jit` (default jit=on makes short OLTP queries pay LLVM compilation that can exceed the query itself — names the reload-context off switch; PostgreSQL), `flush_neighbors` (default 1 coalesces adjacent-page flushes for spinning disks — on SSD/NVMe it only adds latency spikes; names the live SET GLOBAL fix; MySQL), `binlog_row_image` (default FULL logs complete before+after images per UPDATE/DELETE — replica disk/network waste when MINIMAL would do; names the dynamic fix and flashback caveat; MySQL), `random_page_cost` (default 4.0 is the spinning-disk planner cost model — on SSD/NVMe ~1.1 stops index scans being over-discounted; conditional warning naming the ALTER SYSTEM fix; PostgreSQL), `log_buffer_size` (InnoDB WAL buffer overflow evidence via Innodb_log_waits — mid-transaction flushes under write bursts; names the live fix and version caveat; MySQL), `effective_cache_size` (4GB default under-assumes the OS page cache on RAM-heavy hosts — biases plans away from index scans; names the sizing rule and ALTER SYSTEM fix; PostgreSQL), `default_statistics_target` (default ANALYZE sampling depth underestimates skew/high-cardinality columns — misestimates cascade into bad join orders; names the ALTER SYSTEM + re-ANALYZE fix path; PostgreSQL), `sync_binlog` (binlog fsync disabled — a crash can lose commits replicas already received, silently diverging failover targets; names the SET GLOBAL fix and the bounded group-commit alternative; MySQL), `thread_cache_size` (evidence-driven: Threads_created vs Connections miss rate — new connections paying thread-creation cost; suggests a scaled cache size and the flatten-verification step; MySQL), `wal_sender_timeout` (0 disables dead-standby reaping — a crashed replica's slot pins WAL until the disk fills; too-low values kill healthy standbys on flaky links; names the ALTER SYSTEM + reload fix; PostgreSQL), `max_wal_senders` (evidence-driven capacity: live senders + reserved slots counted against the ceiling — 0 disables streaming replication entirely, at-capacity means a replacement standby cannot attach during failover; names the ALTER SYSTEM + restart fix; PostgreSQL), `back_log` (TCP listen backlog — connection bursts beyond it are refused by the kernel before authentication, so load spikes surface as connect-failed storms even with max_connections headroom; names the my.cnf + restart fix since the setting is read-only at runtime; MySQL), `checkpoint_timeout` (<5min = checkpoint storms with full-page writes inflating WAL, >1h = long crash-recovery replay; names the ALTER SYSTEM + pg_reload_conf() fix; PostgreSQL), `ssl_min_protocol_version` (TLSv1/1.1 floors escalated as PCI-DSS-prohibited, and `ssl=off` itself escalated as plaintext connections with the hostssl/pg_hba.conf fix; PostgreSQL), `temp_file_limit` (unlimited per-session spill files escalated as disk-fill blast radius, names the ALTER SYSTEM + pg_reload_conf() fix; PostgreSQL), `tcp_keepalives_idle` (OS-default keepalive escalated — a dead client holds its connection slot for hours; names the 300s reloadable fix; PostgreSQL), `log_lock_waits` (off escalated — lock waits past deadlock_timeout never reach the logs, so post-incident blocking evidence is lost; names the reloadable fix; PostgreSQL), `log_checkpoints` (off on PG≤14 escalated — checkpoint timing and buffer counts invisible, I/O stalls undiagnosable; names the fix and tuning targets), `track_counts` (off escalated — pg_stat counters frozen, autovacuum blind to dead tuples while diagnostics silently read zeros; names the reloadable fix; PostgreSQL), `maintenance_work_mem` (64MB default escalated — VACUUM/CREATE INDEX/ANALYZE spill to temp disk; names the safe raise), `autovacuum_naptime` (raised pass cadence delays cleanup so bloat accumulates between passes; names the fix), `effective_io_concurrency` (spinning-disk default serializes bitmap scans on SSD/NVMe; names the raise), `replication_slots` (inactive slots retaining WAL and exhausted slot capacity block replicas; names the drop command), `slow_query_log` (disabled log or the 10 s default `long_query_time` hides slow queries from every digest view; names the SET GLOBAL fix), `timeout_guards` (PostgreSQL: flags unlimited `statement_timeout` / `idle_in_transaction_session_timeout` so runaway statements and forgotten transactions have an engine-side backstop), `file_access` (MySQL server-side file read/write surface: `local_infile=ON` lets a compromised client have the server read arbitrary files via LOAD DATA LOCAL INFILE, and empty `secure_file_priv` allows unrestricted server-side import/export — each flagged with the exact fix; NULL secure_file_priv called out as fully disabled), `health_audit` (runs every registered configuration audit once and renders a merged report — findings per section with the exact fix, explicit clean summary otherwise), `cancel_query` with `session_id` (pg_cancel_backend / KILL QUERY), plus tracked metrics and static SQL issue suggestions |
